...
\begin{bmatrix}
    W^i & & & b^i\\
	0 & \dots & 0 & c_i
\end{bmatrix} \in \mathbb{R}^{l_i + 1, h_i + 1}  \forall i \in \{0, \dots L\}
\]\\
\[
W'^i=
\begin{bmatrix}
    W^i & b^i
\end{bmatrix} \in \mathbb{R}^{l_i + 1, h_i}  \text{ with } i = L + 1
\]
\end{definition}

\begin{theorem}{Equality of the augumented NN}\\
For all NN with Bias $f_{\theta'}$:\\
$f_{\theta'}(x)=f'_{\theta'}((x,1))$
\end{theorem}

\begin{proof}
    via Induction over L.\\
    For $L=0$...\\
    TODO, but should be pretty trivial
\end{proof}
